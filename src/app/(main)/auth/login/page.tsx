...
export default function LoginPage() {
  ...
  function handleGuestContinue(e: React.FormEvent) {
    ...
    if (!guestName.trim() || !guestEmail.trim() || !guestPhone.trim()) {
      toast.error('이름, 이메일, 연락처를 모두 입력해주세요.')
      return
    }
    const target = new URL(redirectTo, window.location.origin)
    target.searchParams.set('guest', 'true')
    target.searchParams.set('name', guestName.trim())
    target.searchParams.set('email', guestEmail.trim())
    target.searchParams.set('phone', guestPhone.trim())
  ...
  const inputClass = 'w-full rounded-xl border border-zinc-300 pl-10 pr-4 py-2.5 text-sm text-zinc-900 placeholder:text-zinc-400 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20'

  return (
    <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center bg-zinc-50 px-4 py-12">
      <div className="w-full max-w-md">
        <h1 className="text-center text-2xl font-bold text-zinc-900 mb-6">로그인 / 비회원 예약</h1>

        {/* 탭 */}
        <div className="flex rounded-xl border border-zinc-200 bg-zinc-100 p-1 mb-6">
          <button
            type="button"
            onClick={() => setTab('member')}
            className={`flex-1 rounded-lg py-2.5 text-sm font-semibold transition-all ${
              tab === 'member' ? 'bg-white shadow-sm text-zinc-900' : 'text-zinc-500 hover:text-zinc-700'
            }`}
          >
            회원 로그인
          </button>
          <button
            ...
            className={`flex-1 rounded-lg py-2.5 text-sm font-semibold transition-all ${
              tab === 'guest' ? 'bg-white shadow-sm text-zinc-900' : 'text-zinc-500 hover:text-zinc-700'
            }`}
          >
            비회원 예약
          </button>
        ...
        {tab === 'member' ? (
              ...
              <form action={action} className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="email" name="email" required placeholder="example@email.com" className={inputClass} />
                  </div>
                </div>
          ...
          <div className="rounded-2xl border border-zinc-200 bg-white p-8 shadow-sm">
            <p className="text-sm text-zinc-500 mb-6 leading-relaxed">
              회원가입 없이 이름, 이메일, 연락처만 입력하면 바로 예약할 수 있습니다.
            </p>
            <form onSubmit={handleGuestContinue} className="space-y-4">
              <div>
                <label className="mb-1.5 block text-sm font-medium text-zinc-700">이름 <span className="text-red-500">*</span></label>
                <div className="relative">
              ...
              <div>
                <label className="mb-1.5 block text-sm font-medium text-zinc-700">이메일 <span className="text-red-500">*</span></label>
                <div className="relative">
                  <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-zinc-400" />
                  <input
                    type="email"
              ...
              <div>
                <label className="mb-1.5 block text-sm font-medium text-zinc-700">연락처 <span className="text-red-500">*</span></label>
                <div className="relative">
