...
const STOPS = [
  ...
  {
    num: 2,
    name: '서울식물원', name_en: 'Seoul Botanic Park',
    sub: '+8km · 도심 속 열대 온실', sub_en: '+8km · Tropical greenhouse in the city',
    photo: '/stop-ara-botanic.png',
    badge: { text: '8km', color: 'bg-sky-500' },
  },
  {
    num: 3,
    name: '아라항', name_en: 'Ara Port',
    sub: '+6.5km · 수상레저 & 선착장', sub_en: '+6.5km · Watercraft & marina',
    photo: '/stop-ara-port.png',
    badge: { text: '14.5km', color: 'bg-violet-500' },
  },
  {
    num: 4,
    name: '아라마루 전망대', name_en: 'Ara Maru Observatory',
    sub: '+9.5km · 원형 스카이워크', sub_en: '+9.5km · Circular skywalk',
    photo: '/stop-ara-tower.png',
    badge: { text: '24km', color: 'bg-rose-500' },
  },
  {
    num: 5,
    name: '아라뱃길 인공폭포', name_en: 'Ara Waterway Waterfall',
    sub: '아라마루 전망대 인근', sub_en: 'Near Ara Maru Observatory',
    photo: '/stop-ara-waterfall.png',
    badge: { text: '🚴 RIDING', color: 'bg-amber-400 text-zinc-900' },
    special: true,
  },
  {
    num: 6,
    name: '아라뱃길', name_en: 'Ara Waterway',
    sub: '서해로 이어지는 자전거도로', sub_en: 'Cycling path to the West Sea',
    photo: '/stop-ara-bikeway.png',
    badge: { text: 'BIKEWAY', color: 'bg-teal-500' },
  },
  {
    num: 7,
    name: '정서진 (서해갑문)', name_en: 'Jeongseojin (West Sea Lock)',
    sub: '+10.5km · 서해 낙조 포인트', sub_en: '+10.5km · Yellow Sea sunset point',
    photo: '/stop-ara-jeongseojin.png',
    badge: { text: '34.5km', color: 'bg-orange-500' },
  },
  {
    num: 8,
    name: '당산역 귀환', name_en: 'Return to Dangsan',
    sub: 'FINISH · 왕복 69km 완주! 🎉', sub_en: 'FINISH · 69km round trip complete! 🎉',
    photo: '/stop-ara-dangsan.png',
    ...
    >
      ...
      {stop.special && (
        <div className="absolute top-0 right-0 overflow-hidden w-24 h-24">
          <div className="absolute top-4 -right-6 bg-amber-400 text-zinc-900 text-[10px] font-black px-8 py-1 rotate-45 shadow-lg">SCENE</div>
        </div>
      )}
...
export default function AraRouteSection() {
  const locale = useLocale()
  const isKo = locale === 'ko'
    ...
    <section className="mt-10">
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-2 rounded-full bg-zinc-900 px-4 py-2">
          ...
          <span className="text-sm font-black text-white">{isKo ? '왕복 69km' : '69km round trip'}</span>
          <span className="text-xs text-zinc-400">{isKo ? '/ 편도 34.5km' : '/ 34.5km one way'}</span>
        </div>
      </div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-4 text-xs text-zinc-500">
        <span className="flex items-center gap-1.5">
          <span className="h-2 w-2 rounded-full bg-orange-400" />{isKo ? '편도 종점 34.5km' : 'One-way 34.5km'}
        </span>
        <span className="flex items-center gap-1.5">
          <span className="h-2 w-2 rounded-full bg-emerald-400" />{isKo ? '왕복 완주 69km' : 'Round trip 69km'}
        </span>
