...
const STOPS = [
  ...
  {
    num: 2,
    name: '난지 하늘공원', name_en: 'Nanjido Sky Park',
    sub: '+5km · 꽃밭 & 메타세쿼이아 숲', sub_en: '+5km · Flower fields & metasequoia forest',
    photo: '/stop-haengju-nanjido.png',
    badge: { text: '5km', color: 'bg-sky-500' },
  },
  {
    num: 3,
    name: '행주산성', name_en: 'Haengju Fortress',
    sub: '+6.5km · 역사 유적지', sub_en: '+6.5km · Historic fortress site',
    photo: '/stop-haengju-fortress.png',
    badge: { text: '11.5km', color: 'bg-violet-500' },
  },
  {
    num: 4,
    name: '아라항 (현대아울렛)', name_en: 'Ara Port (Hyundai Outlet)',
    sub: '+6km · 수상레저 & 쇼핑', sub_en: '+6km · Watercraft & shopping',
  ...
  },
  {
    ...
    sub: '+6km · 도심 속 열대 온실', sub_en: '+6km · Tropical greenhouse in the city',
    photo: '/stop-ara-botanic.png',
    badge: { text: '23.5km', color: 'bg-teal-500' },
  },
  {
    num: 6,
    name: '선유도공원', name_en: 'Seonyudo Island Park',
    sub: '+7km · 한강 섬 공원', sub_en: '+7km · Han River island park',
  ...
  },
  {
    ...
    sub: 'FINISH · 32km 완주! 🎉', sub_en: 'FINISH · 32km complete! 🎉',
    photo: '/stop-ara-dangsan.png',
    ...
    >
      ...
      {stop.special && (
        <div className="absolute top-0 right-0 overflow-hidden w-24 h-24">
          <div className="absolute top-4 -right-6 bg-amber-400 text-zinc-900 text-[10px] font-black px-8 py-1 rotate-45 shadow-lg">FREE!</div>
        </div>
      )}
      ...
      <div className={`absolute inset-0 bg-zinc-950/60 backdrop-blur-[2px] flex items-center justify-center transition-opacity duration-200 ${hovered ? 'opacity-100' : 'opacity-0'}`}>
        <div className="text-center px-4">
          ...
          {stop.special && <p className="text-sm font-bold text-amber-400 mt-2">{specialText}</p>}
        </div>
      </div>
    </div>
  )
